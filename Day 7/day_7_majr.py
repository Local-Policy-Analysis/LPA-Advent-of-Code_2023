...
def handcardval(h):
    ...
    return int( ''.join( str(x) for x in sorted( [cardvals.index(c) for c in h[0]],  reverse=True)))
...
def cmp(l, r):
    ...
    return ul[0] != sum(ul)/2 and (ul.index(min(ul)) > 0 and -1 or 1)  or c[0] != sum(c)/2 and (c.index(max(c)) > 0 and -1 or 1) or bvc[0] != sum(bvc)/2 and (bvc.index(max(bvc)) > 0 and -1 or 1) or 0


from functools import cmp_to_key as ck
ranks = sorted(lines, key=ck(cmp), reverse=True)
